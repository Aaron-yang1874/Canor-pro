...
async function handlePOST(request: NextRequest) {
  ...
  const { modalities, textPrompt, imageData, audioData, videoData, fusionStrategy, modalityWeights, prompt, numIterations } = body;
  ...
  const fusionResult = fuseMultimodal(multimodalInput);
  const generationResult = zeroShotGeneration({
    prompt: prompt || "generate from media",
    modality: validMods[0] || "text",
    numIterations: typeof numIterations === "number" ? numIterations : 3,
    styleNeighbors: 5,
    qualityThreshold: 0.7,
    interpolationWeights: [0.5, 0.3, 0.2],
  });
  return NextResponse.json({ fusion: fusionResult, generation: generationResult });
}
